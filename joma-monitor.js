...
// Joma monitor uses built-in fetch (Node 18+)
const STATE_FILE = path.join(__dirname, "state-joma.json");
const EVENTS_FILE = path.join(__dirname, "events-joma.json");
...
// ---------------- SCRAPER ----------------
// Joma uses product tiles with data-product-id and price blocks.

async function fetchJomaPage(page = 1) {
  const url = `${BASE_URL}/perfume-fragrances.html?p=${page}`;

  const res = await fetch(url, {
    headers: {
      "User-Agent": "Mozilla/5.0 (joma-monitor bot)"
    }
  });

  if (!res.ok) {
    console.warn(`Joma page ${page} failed: ${res.status}`);
  ...
  const items = html
    ...
    .map(block => {
      ...
      const nameMatch = block.match(/class="product-item-name"[^>]*>([^<]+)/);
  ...
  const hasMore = html.includes('class="pages-item-next"');

  return { products: items, hasMore };
}

async function fetchAllJoma() {
  let all = [];
  let page = 1;

  while (true) {
    const { products, hasMore } = await fetchJomaPage(page);
    all.push(...products);
    if (!hasMore) break;
    page++;
    await sleep(1000);
...
(async function main() {
  ...
  console.log("Checking Joma...");
  const products = await fetchAllJoma();
  ...
  if (state.firstRun) {
    console.log("Joma baseline established.");
    state.firstRun = false;
  } else {
    console.log(`${events.length} Joma event(s) found.`);
