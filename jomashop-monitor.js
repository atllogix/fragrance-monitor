...
const PAGE_SIZE = 60;
const NTFY_TOPIC = process.env.NTFY_TOPIC; // same repo secret as the Shopify monitor
const STATE_FILE = path.join(__dirname, 'jomashop-state.json');
const HISTORY_FILE = path.join(__dirname, 'history.md'); // shared log with the Shopify monitor
const PAGE_DELAY_MS = 500;
const SAFETY_MAX_PAGES = 1000;

// Optional filters — same idea as the Shopify monitor. Leave as-is to catch everything.
const BRAND_WATCHLIST = []; // e.g. ['Shauran', 'Creed'] — empty array = notify for all brands
const MIN_DISCOUNT_PCT = 0; // e.g. 20 = only notify price drops of 20% or more
